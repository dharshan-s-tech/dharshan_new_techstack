...
export async function POST(req: NextRequest) {
  try {
    ...
    const allowedFileExts = ['pdf', 'doc', 'docx', 'xls', 'xlsx', 'zip', 'png', 'jpg', 'jpeg', 'webp'];
    ...
    const folder = allowedImageExts.includes(ext) || type === 'image' ? 'images' : 'uploads';
    ...
    return NextResponse.json({ url, filename, name: file.name, size: file.size });
  } catch (err: any) {
