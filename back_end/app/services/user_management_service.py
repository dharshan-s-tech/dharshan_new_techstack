...
class UserManagementService:
    ...
    def get_users(
        ...
        formatted = []
        for u in rows:
            name_disp = clean_json_str(u.full_name) or clean_json_str(u.name) or f"{clean_json_str(u.first_name)} {clean_json_str(u.last_name)}".strip() or u.username or "ΓÇö"
            formatted.append({
                "id": str(u.id),
                ...
                "mobile": u.mobile or "",
                "role_id": u.role_id,
                "role_name": role_map.get(u.role_id, f"Role #{u.role_id}" if u.role_id else "ΓÇö"),
                "wings_id": u.wings_id,
                "wing_title": wing_map.get(u.wings_id, "ΓÇö") if u.wings_id else "ΓÇö",
    ...
    def get_user_by_id(db: Session, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "mobile": u.mobile or "",
            "role_id": u.role_id,
            "role_name": clean_json_str(role.name) if role else "ΓÇö",
            "wings_id": u.wings_id,
            "wing_title": wing.title if wing else "ΓÇö",
    ...
    @staticmethod
    def get_roles(db: Session, page: int = 1, limit: int = 50, search: Optional[str] = None, website_id: Optional[str] = None, status: Optional[str] = None, role_id: Optional[str] = None, **kwargs) -> Dict[str, Any]:
        query = db.query(Role)
        if role_id and role_id != "all":
            try:
    ...
    def get_audit_trail_logs(db: Session, page: int = 1, limit: int = 20, search: Optional[str] = None) -> Dict[str, Any]:
        ...
        for log in rows:
            formatted.append({
                ...
                "table_alias": log.table_alias or "ΓÇö",
    ...
    @staticmethod
    def get_role_permissions(db: Session, page: int = 1, limit: int = 50, search: Optional[str] = None, website_id: Optional[str] = None, role_id: Optional[str] = None, **kwargs) -> Dict[str, Any]:
