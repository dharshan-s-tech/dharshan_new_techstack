...
def print_richest_records():
    with engine.connect() as conn:
        topics = [
            ...
            ("Auditing Standards", ["%standard%"]),
            ("Citizen Charter", ["%citizen%"]),
            ("Overview", ["%overview%"]),
            ("International Relations", ["%international%", "%global%", "%intosai%", "%asosai%", "%bilateral%"]),
        ...
        for title, patterns in topics:
            ...
            query = f"""
                SELECT p.id, p.slug, p.title, 
                       ...
                       COALESCE(NULLIF(p.file_title, ''), '') as file_title,
                ...
                GROUP BY p.id, p.slug, p.title, p.content, p.file_title, p.upload_file
                ORDER BY GREATEST(
            ...
            for r in rows:
                print(f"  ID: {r['id']:<5} | P_len: {r['p_len']:<6} | EN_len: {r['en_len']:<6} | HI_len: {r['hi_len']:<6} | File: {r['upload_file']:<25} | Slug: {r['slug']:<45} | Title: {r['title'][:35]}")

if __name__ == '__main__':
    print_richest_records()
